...
public class RungeKuttaGill extends BaseEngRunGillKutt {
    ...
    protected double deltaY(double xn, double yn, double h) {
        ...
        double k3 = h * function(xn + h/2, yn + (Math.sqrt(2.0)-1.0) / 2.0 * k1 + (1.0-Math.sqrt(2.0)) / 2.0 * k2, example);
        double k4 = h * function(xn + h, yn - Math.sqrt(2.0) / 2 * k2 + (2 + Math.sqrt(2)) / 2 * k3, example);
